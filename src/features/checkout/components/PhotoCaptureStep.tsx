...
interface PhotoCaptureStepProps {
  ...
  error: string | null;
  // Optional top chrome (checkout renders SecondaryNav + progress bar; retake none).
  chrome?: ReactNode;
  // Optional "skip this step" affordance (checkout only).
  skip?: { onConfirm: () => void; isSkipping: boolean };
  // Optional already-uploaded photo to review with a "Continue" (checkout only).
  existing?: {
...
/**
 * The shared ID / selfie photo-capture UI (landing → capture/select → review →
 * confirm) used by both the checkout upload steps and the post-approval retake
 * flow. It owns the capture state; the data operations + chrome come in as props.
 */
export const PhotoCaptureStep = ({
  ...
  const showExisting = !!existing?.photoUrl && !retaken;
  const displayUrl = previewUrl ?? (showExisting ? existing?.photoUrl : null);
  ...
  const showLoader = !!existing?.isLoading && !previewUrl && !retaken;
    ...
    <>
        ...
        <div className="mx-auto w-full max-w-xl">
          ...
          {showLoader ? (
            ...
            <>
              ...
              <h1 className={`text-2xl font-semibold text-text-primary ${skip ? "mt-6" : ""}`}>
                {c.landingTitle}
              </h1>
            ...
            <>
              ...
              <div className="relative mt-12 overflow-hidden rounded-2xl bg-black">
                ...
                <img src={displayUrl} alt="Your photo" className="mx-auto max-h-[400px] w-full object-contain" />
                <button
                  type="button"
                  onClick={retake}
              ...
              {previewUrl ? (
                ...
                <Button
                  ...
                  size="lg"
                  fullWidth
                  loading={existing?.isConfirming}
                  onClick={() => existing?.onConfirm()}
                  className="mt-10"
                >
                  Continue
                </Button>
      ...
      {skip && (
        <SkipPhotoDrawer
          ...
          onConfirm={skip.onConfirm}
          isSkipping={skip.isSkipping}
        />
      )}
    </>
  );
