...
// Match AUM's "Sildenafil 25mg (Generic Viagra)": simple drug name + dosage
// value (no space before "mg") and the generic brand as a separate part so the
// brand can drop cleanly onto its own line on mobile (AUM stacks it at ≤991px).
...
const CouponSection = ({
  ...
  if (cart.discounts.length > 0) {
    ...
    return (
      <div className="flex items-center justify-between gap-3">
        ...
        {amount > 0 && <span className="font-medium text-save">- {money(amount)}</span>}
      </div>
    );
  }
  ...
  return (
    <div>
      {/* The "I have a coupon" row stays visible; checking it fills the ring
          green and collapses the input open below (matches AUM). */}
      <button
        type="button"
        onClick={() => {
          setOpen((o) => !o);
...
export const OrderVerificationPage = () => {
  ...
  const product = cart ? productNameParts(cart) : null;
            ...
            <div className="mt-6 rounded-2xl bg-bg-card p-[30px] shadow-sm max-[900px]:p-4 max-[436px]:p-3">
                ...
                <>
                  ...
                  <div className="py-[5px]">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <FreeOrPrice value={cart.final_delivery_price} />
                    </div>
                    {showAddressWarning && (
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="flex items-baseline gap-2">
                  ...
                  <span className="text-[18px] font-bold text-text-primary">{money(displayTotal)}</span>
                </span>
              </div>
