...
const DeliverySummary = ({ option, cutoff }: { option: DeliveryOption; cutoff: string | null }) => (
  <>
...
/**
 * Order-scoped `/order-shipping-confirmation`. Matches the checkout
 * ShippingConfirmationPage; patient info is read-only (no "Change") in the
 * post-approval pay flow.
 */
    ...
    <>
      ...
      <main className="min-h-screen bg-bg-main px-4 py-10">
        <div className="mx-auto w-full max-w-xl">
          ...
          <p className="mt-1 text-text-muted">Review your shipping and delivery before paying</p>
          ...
          {isLoading ? (
            ...
            <div className="mt-6 rounded-2xl bg-bg-card p-5 shadow-sm">
              <div className="flex flex-col gap-4">
                ...
                <Card title="Shipping Information" onChange={changeShipping}>
                  {address && (
                    <>
                      <p className="text-text-muted">
                        {address.street_1}
                ...
                {deliveryOption && (
                  <Card title="Delivery Option" onChange={changeDelivery}>
              ...
              <button
                type="button"
                onClick={onContinue}
                className="mt-5 w-full cursor-pointer rounded-full bg-[#e05c4b] py-3.5 text-sm font-semibold uppercase tracking-wide text-white transition-colors hover:opacity-90"
              >
                Continue
              </button>
            </div>
